...
% 1. Lequel des prédicats suivants permet de déterminer si en arrivant à une certaine heure, il est
% possible d’attraper en temps un vol donné sachant qu’il faut être arrivé au moins 60 minutes à
% l’avance.
...
% 2. Créer les opérateurs permettant l’écriture suivante:

% ?- vol(montreal,sherbrooke) arrive_a X.
% X=17:10

arrive_a(vol(X,Y),Z) :- flight(X,Y,Z,_).
